...
async function validateTextDocument(textDocument) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (errorIndex !== -1) {
            diagnostics.push(node_1.Diagnostic.create({ start: { line: i, character: errorIndex },
                end: { line: i, character: errorIndex + 5 } }, 'Found error keyword in line', node_1.DiagnosticSeverity.Error, 100, 'VixChecker'));
        }
        // 检查语法错误：未闭合的括号
        ...
        if (openParen > closeParen) {
            diagnostics.push(node_1.Diagnostic.create({ start: { line: i, character: 0 },
                end: { line: i, character: maskedLine.length } }, 'Unmatched parentheses', node_1.DiagnosticSeverity.Warning, 101, 'VixChecker'));
        }
    ...
    if (maskState.inDoubleQuote && doubleQuoteStart) {
        diagnostics.push(node_1.Diagnostic.create({
            start: doubleQuoteStart,
            end: { line: lines.length - 1, character: lines[lines.length - 1]?.length || 0 }
        }, 'Unterminated double-quoted string', node_1.DiagnosticSeverity.Warning, 103, 'VixChecker'));
    }
    if (maskState.inSingleQuote && singleQuoteStart) {
        diagnostics.push(node_1.Diagnostic.create({
            start: singleQuoteStart,
            end: { line: lines.length - 1, character: lines[lines.length - 1]?.length || 0 }
        }, 'Unterminated single-quoted string', node_1.DiagnosticSeverity.Warning, 102, 'VixChecker'));
    }
    // 发送诊断信息到客户端
